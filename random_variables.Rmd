...
set.seed(1)
n<-1000
samp_mean_5 <- vector("numeric",n)
for (i in 1:n) {
  samp_mean_5[i]<-mean(sample(x,5,replace=TRUE))
}
diff_mean_5<-abs(samp_mean_5-mean(x))
length(which(diff_mean_5>1))/length(diff_mean_5)
```
...
set.seed(1)
n<-1000
samp_mean_50 <- vector("numeric",n)
for (i in 1:n) {
  samp_mean_50[i]<-mean(sample(x,50,replace=TRUE))
}
diff_mean_50<-abs(samp_mean_50-mean(x))
length(which(diff_mean_50>1))/length(diff_mean_50)
```
...
set.seed(1)
df <- data.frame(sample_size_5 = samp_mean_5,
                 sample_size_50 = samp_mean_50)
...
```{r}
length(which(samp_mean_50>=23 & samp_mean_50<=25))/length(samp_mean_50)
```
